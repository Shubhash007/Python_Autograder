...
def evaluate_code_ast(code, rubric):
    ...
    def contains_fibonacci_approach(node):
        return any(isinstance(n, (ast.For, ast.While, ast.FunctionDef)) for n in ast.walk(node))
    ...
    if contains_fibonacci_approach(tree):
        results["Marks"][1] = rubric["Marks"][1]

    if not contains_hardcoded_fibonacci(tree):
