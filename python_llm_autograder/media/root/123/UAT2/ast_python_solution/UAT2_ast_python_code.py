...
def evaluate_code_ast(code, rubric):
    ...
    def check_for_loop_and_int(node):
    ...
    def check_list_comprehension_and_int(node):
    ...
    def check_weights_list(node):
        return isinstance(node, ast.List) and [elt.n for elt in node.elts] == [2, 7, 6, 5, 4, 3, 2]

    def check_for_loop_and_multiplication(node):
        if isinstance(node, ast.For):
            for inner_node in ast.walk(node):
    ...
    def check_list_comprehension_and_multiplication(node):
        if isinstance(node, ast.ListComp):
            for inner_node in ast.walk(node):
                if isinstance(inner_node, ast.BinOp) and isinstance(inner_node.op, ast.Mult):
                    return True
    ...
    def check_checksum_list_s_f(node):
        return isinstance(node, ast.List) and [elt.s for elt in node.elts] == ['J','Z','I','H','G','F','E','D','C','B','A']

    def check_checksum_list_t_g(node):
        return isinstance(node, ast.List) and [elt.s for elt in node.elts] == ['G', 'F', 'E', 'D', 'C', 'B', 'A', 'J', 'Z', 'I', 'H']
    ...
    for node in ast.walk(tree):
        ...
        if check_for_loop_and_int(node):
            results["Marks"][3] = rubric["Marks"][3]
        if check_list_comprehension_and_int(node):
            results["Marks"][4] = rubric["Marks"][4]
        if check_weights_list(node):
            results["Marks"][5] = rubric["Marks"][5]
        if check_for_loop_and_multiplication(node):
            results["Marks"][6] = rubric["Marks"][6]
        if check_list_comprehension_and_multiplication(node):
