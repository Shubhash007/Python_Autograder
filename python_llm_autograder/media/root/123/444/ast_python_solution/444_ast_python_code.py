...
def evaluate_code_ast(code, rubric):
    ...
    def check_weights_list(node):
        return isinstance(node, ast.List) and all(isinstance(elt, ast.Constant) for elt in node.elts) and \
               [elt.value for elt in node.elts] == [2, 7, 6, 5, 4, 3, 2]
    ...
    def check_checksum_list_s_f(node):
        return isinstance(node, ast.List) and all(isinstance(elt, ast.Constant) for elt in node.elts) and \
               [elt.value for elt in node.elts] == ['J','Z','I','H','G','F','E','D','C','B','A']

    def check_checksum_list_t_g(node):
        return isinstance(node, ast.List) and all(isinstance(elt, ast.Constant) for elt in node.elts) and \
               [elt.value for elt in node.elts] == ['G', 'F', 'E', 'D', 'C', 'B', 'A', 'J', 'Z', 'I', 'H']
